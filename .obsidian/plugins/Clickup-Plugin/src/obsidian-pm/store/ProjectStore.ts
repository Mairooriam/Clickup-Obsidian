...
import {
  importTasksFromMarkdown as bridgeImport,
  exportTasksToMarkdown as bridgeExport,
  type TaskParserBridgeOptions
} from './TaskParserBridge'
...
export class ProjectStore {
  ...
  /**
   * Parse a TaskParser-format markdown string and insert the resulting tasks
   * into a project, optionally under a specific parent task.
   *
   * @param project   - The target project.
   * @param md        - Markdown string in TaskParser format.
   * @param parentId  - ID of the parent task, or null for top-level.
   * @param options   - Controls how `completed` maps to PM status strings.
   * @returns         The newly inserted top-level Task objects.
   */
  async importTasksFromMarkdown(
    project: Project,
    md: string,
    parentId: string | null = null,
    options?: TaskParserBridgeOptions
  ): Promise<Task[]> {
    const imported = bridgeImport(md, options)
  ...
  /**
   * Serialize the project's task tree into a TaskParser-format markdown string.
   * Useful for exporting to ClickUp or other markdown-based systems.
   *
   * @param project - The project to export.
   * @param options - Controls how PM status maps to `completed`.
   * @returns       Markdown string in TaskParser format.
   */
  exportTasksToMarkdown(project: Project, options?: TaskParserBridgeOptions): string {
    return bridgeExport(project.tasks, options)
  }
}
